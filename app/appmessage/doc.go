...
supported Kalibrium messages to and from the appmessage. This package does not deal
with the specifics of message handling such as what to do when a message is
received. This provides the caller with a high level of flexibility.

# Kalibrium Message Overview

The Kalibrium protocol consists of exchanging messages between peers. Each
message is preceded by a header which identifies information about it such as
which Kalibrium network it is a part of, its type, how big it is, and a checksum
...
of the currently supported Kalibrium messages are provided. For these supported
messages, all of the details of marshalling and unmarshalling to and from the
appmessage using Kalibrium encoding are handled so the caller doesn't have to concern
themselves with the specifics.

# Message Interaction

The following provides a quick summary of how the Kalibrium messages are intended
...
and write Kalibrium messages. The following sections provide a quick overview of
...
The Kalibrium network is a magic number which is used to identify the start of a
message and which Kalibrium network the message applies to. This package provides
...
As discussed in the Kalibrium message overview section, this package reads
and writes Kalibrium messages using a generic interface named Message. In
...
In order to unmarshall Kalibrium messages from the appmessage, use the ReadMessage
function. It accepts any io.Reader, but typically this will be a net.Conn to
a remote node running a Kalibrium peer. Example syntax is:
	...
	msg, rawPayload, err := appmessage.ReadMessage(conn, pver, kalibriumNet)
	if err != nil {
...
In order to marshall Kalibrium messages to the appmessage, use the WriteMessage
function. It accepts any io.Writer, but typically this will be a net.Conn to
a remote node running a Kalibrium peer. Example syntax to request addresses
from a remote peer is:
	...
	// error.
	err := appmessage.WriteMessage(conn, msg, pver, kalibriumNet)
	if err != nil {
		// Log and handle the error
	}
