...
// Outpoint defines a Kalibrium data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new Kalibrium transaction outpoint point with the
...
// TxIn defines a Kalibrium transaction input.
type TxIn struct {
...
// NewTxIn returns a new Kalibrium transaction input with the provided
...
// TxOut defines a Kalibrium transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new Kalibrium transaction output with the provided
...
// MsgTx implements the Message interface and represents a Kalibrium tx message.
