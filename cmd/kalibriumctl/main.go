...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		kalibriumdMessage, err := client.Post(&protowire.KalibriumdMessage{Payload: &protowire.KalibriumdMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := kalibriumdMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	kalibriumdMessage := &protowire.KalibriumdMessage{}
	err := protojson.Unmarshal([]byte(response), kalibriumdMessage)
	if err != nil {
	...
	return marshalOptions.Format(kalibriumdMessage)
}
