...
// 2. A wrapper that implements isKalibriumdMessage_Payload, having a single field pointing to the concrete command
//    bearing the name of the command with `KalibriumdMessage_` prefix and `Request` suffix
...
// generateKalibriumdMessage generates a wrapped KalibriumdMessage with the given `commandValue`
func generateKalibriumdMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.KalibriumdMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	kalibriumdMessage := reflect.New(reflect.TypeOf(protowire.KalibriumdMessage{}))
	kalibriumdMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return kalibriumdMessage.Interface().(*protowire.KalibriumdMessage), nil
}
