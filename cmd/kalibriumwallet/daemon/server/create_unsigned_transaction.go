...
func (s *server) createUnsignedTransactions(address string, amount uint64, isSendAll bool, fromAddressesString []string, useExistingChangeAddress bool) ([][]byte, error) {
	...
	selectedUTXOs, spendValue, changeEquil, err := s.selectUTXOs(amount, isSendAll, feePerInput, fromAddresses)
	if err != nil {
		return nil, err
	}
	...
	unsignedTransaction, err := libkalibriumwallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func (s *server) selectUTXOs(spendAmount uint64, isSendAll bool, feePerInput uint64, fromAddresses []*walletAddress) (
	selectedUTXOs []*libkalibriumwallet.UTXO, totalReceived uint64, changeEquil uint64, err error) {
	...
	if dagInfo.NetworkName == "Kalibrium-testnet-11" {
	...
	if totalValue < totalSpend {
		return nil, 0, 0, errors.Errorf("Insufficient funds for send: %f required, while only %f available",
			float64(totalSpend)/constants.EquilPerKalibrium, float64(totalValue)/constants.EquilPerKalibrium)
	}
