...
func parse(conf *parseConfig) error {
	...
	for i, transaction := range transactions {
		...
		for index, input := range partiallySignedTransaction.Tx.Inputs {
			...
			if conf.Verbose {
				fmt.Printf("Input %d: \tOutpoint: %s:%d \tAmount: %.2f Kalibrium\n", index, input.PreviousOutpoint.TransactionID,
					input.PreviousOutpoint.Index, float64(partiallySignedInput.PrevOutput.Value)/float64(constants.EquilPerKalibrium))
			}

			allInputEquil += partiallySignedInput.PrevOutput.Value
		}
		if conf.Verbose {
			fmt.Println()
		}

		allOutputEquil := uint64(0)
		for index, output := range partiallySignedTransaction.Tx.Outputs {
			...
			fmt.Printf("Output %d: \tRecipient: %s \tAmount: %.2f Kalibrium\n",
				index, addressString, float64(output.Value)/float64(constants.EquilPerKalibrium))

			allOutputEquil += output.Value
		}
		fmt.Println()

		fmt.Printf("Fee:\t%d Equil\n\n", allInputEquil-allOutputEquil)
	}
