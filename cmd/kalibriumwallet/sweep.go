...
func sweep(conf *sweepConfig) error {
	...
	publicKeybytes, err := libkalibriumwallet.PublicKeyFromPrivateKey(privateKeyBytes)
	if err != nil {
		return err
	}
	...
	UTXOs, err := libkalibriumwallet.KalibriumwalletdUTXOsTolibkalibriumwalletUTXOs(getExternalSpendableUTXOsResponse.Entries)
	if err != nil {
		return err
	}
	...
	for i, txID := range response.TxIDs {
		fmt.Printf("\t%s\n", txID)
		fmt.Println("\tSwept:\t", utils.FormatKali(splitTransactions[i].Outputs[0].Value), " KALI")
		totalExtracted = totalExtracted + splitTransactions[i].Outputs[0].Value
	}

	fmt.Println("\nTotal Funds swept (including transaction fees):")
	fmt.Println("\t", utils.FormatKali(totalExtracted), " KALI")
