...
// Takes in a string representation of the KALI value to convert to Equil
func TestKaliToEquil(t *testing.T) {
	...
	for _, currentTestVector := range validCases {
		convertedAmount, err := KaliToEquil(currentTestVector.originalAmount)
	...
	for _, currentTestVector := range invalidCases {
		_, err := KaliToEquil(currentTestVector)
...
func TestValidateAmountFormat(t *testing.T) {
	validCases := []string{
		...
		"184467440737.09551615", // Maximum input that can be represented in equil later
		"184467440737.09551616", // Cannot be represented in equil, but we'll acccept for "correct format"
		"999999999999.99999999", // Cannot be represented in equil, but we'll acccept for "correct format"
	}

	for _, testCase := range validCases {
		err := validateKALIAmountFormat(testCase)
