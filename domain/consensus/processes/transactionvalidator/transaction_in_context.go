...
func (v *transactionValidator) ValidateTransactionInContextAndPopulateFee(stagingArea *model.StagingArea,
	...
	totalEquilIn, err := v.checkTransactionInputAmounts(tx)
	if err != nil {
		return err
	}

	totalEquilOut, err := v.checkTransactionOutputAmounts(tx, totalEquilIn)
	if err != nil {
		return err
	}

	tx.Fee = totalEquilIn - totalEquilOut
...
func (v *transactionValidator) checkTransactionInputAmounts(tx *externalapi.DomainTransaction) (totalEquilIn uint64, err error) {
	totalEquilIn = 0
	...
	for _, input := range tx.Inputs {
		...
		// a transaction are in a unit value known as a equil. One
		// Kalibrium is a quantity of equil as defined by the
		// EquilPerKalibrium constant.
		totalEquilIn, err = v.checkEntryAmounts(utxoEntry, totalEquilIn)
		if err != nil {
			return 0, err
		}
	}
	...
	return totalEquilIn, nil
}

func (v *transactionValidator) checkEntryAmounts(entry externalapi.UTXOEntry, totalEquilInBefore uint64) (totalEquilInAfter uint64, err error) {
	// The total of all outputs must not be more than the max
	// allowed per transaction. Also, we could potentially overflow
	// the accumulator so check for overflow.

	originTxEquil := entry.Amount()
	totalEquilInAfter = totalEquilInBefore + originTxEquil
	if totalEquilInAfter < totalEquilInBefore ||
		totalEquilInAfter > constants.MaxEquil {
		return 0, errors.Wrapf(ruleerrors.ErrBadTxOutValue, "total value of all transaction "+
			"inputs is %d which is higher than max "+
			"allowed value of %d", totalEquilInBefore,
			constants.MaxEquil)
	}
	return totalEquilInAfter, nil
}

func (v *transactionValidator) checkTransactionOutputAmounts(tx *externalapi.DomainTransaction, totalEquilIn uint64) (uint64, error) {
	totalEquilOut := uint64(0)
	// Calculate the total output amount for this transaction. It is safe
	// to ignore overflow and out of range errors here because those error
	// conditions would have already been caught by checkTransactionAmountRanges.
	for _, output := range tx.Outputs {
		totalEquilOut += output.Value
	}

	// Ensure the transaction does not spend more than its inputs.
	if totalEquilIn < totalEquilOut {
		return 0, errors.Wrapf(ruleerrors.ErrSpendTooHigh, "total value of all transaction inputs for "+
			...
			"spent of %d", totalEquilIn, totalEquilOut)
	}
	return totalEquilOut, nil
}
