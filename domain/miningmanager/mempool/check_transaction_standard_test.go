...
func TestIsTransactionOutputDust(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			"max equil amount is never dust",
			externalapi.DomainTransactionOutput{Value: constants.MaxEquil, ScriptPublicKey: scriptPublicKey},
			util.Amount(1000),
			false,
		},
		{
...
func TestCheckTransactionStandardInIsolation(t *testing.T) {
	...
	addr, err := util.NewAddressPublicKey(addrHash[:], util.Bech32PrefixKalibriumTest)
	if err != nil {
