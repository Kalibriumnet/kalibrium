...
func (uis *utxoIndexStore) commit() error {
	...
	for scriptPublicKeyString, toRemoveUTXOOutpointEntryPairs := range uis.toRemove {
		...
		for outpointToRemove, utxoEntryToRemove := range toRemoveUTXOOutpointEntryPairs {
			...
			toRemoveEquilSupply = toRemoveEquilSupply + utxoEntryToRemove.Amount()
		}
	}

	toAddEquilSupply := uint64(0)
	...
	for scriptPublicKeyString, toAddUTXOOutpointEntryPairs := range uis.toAdd {
		...
		for outpointToAdd, utxoEntryToAdd := range toAddUTXOOutpointEntryPairs {
			...
			toAddEquilSupply = toAddEquilSupply + utxoEntryToAdd.Amount()
		}
	}
	...
	err = uis.updateCirculatingEquilSupply(dbTransaction, toAddEquilSupply, toRemoveEquilSupply)
	if err != nil {
		return err
	}
...
func (uis *utxoIndexStore) addAndCommitOutpointsWithoutTransaction(utxoPairs []*externalapi.OutpointAndUTXOEntryPair) error {
	...
	for _, pair := range utxoPairs {
		...
		toAddEquilSupply = toAddEquilSupply + pair.UTXOEntry.Amount()
	}

	err := uis.updateCirculatingEquilSupplyWithoutTransaction(toAddEquilSupply, uint64(0))
	if err != nil {
		return err
	}
...
func (uis *utxoIndexStore) initializeCirculatingEquilSupply() error {
	...
	circulatingEquilSupplyInDatabase := uint64(0)
	for cursor.Next() {
		serializedUTXOEntry, err := cursor.Value()
		if err != nil {
			return err
		...
		circulatingEquilSupplyInDatabase = circulatingEquilSupplyInDatabase + utxoEntry.Amount()
	}

	err = uis.database.Put(
		circulatingSupplyKey,
		binaryserialization.SerializeUint64(circulatingEquilSupplyInDatabase),
	)
...
func (uis *utxoIndexStore) updateCirculatingEquilSupply(dbTransaction database.Transaction, toAddEquilSupply uint64, toRemoveEquilSupply uint64) error {
	if toAddEquilSupply != toRemoveEquilSupply {
		...
		err = dbTransaction.Put(
			circulatingSupplyKey,
			binaryserialization.SerializeUint64(circulatingSupply+toAddEquilSupply-toRemoveEquilSupply),
		)
		if err != nil {
			return err
		}
...
func (uis *utxoIndexStore) updateCirculatingEquilSupplyWithoutTransaction(toAddEquilSupply uint64, toRemoveEquilSupply uint64) error {
	if toAddEquilSupply != toRemoveEquilSupply {
...
func (uis *utxoIndexStore) getCirculatingEquilSupply() (uint64, error) {
	if uis.isAnythingStaged() {
