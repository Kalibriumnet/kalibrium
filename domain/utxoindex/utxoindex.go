...
func (ui *UTXOIndex) Reset() error {
	...
	err = ui.store.initializeCirculatingEquilSupply() //At this point the database is empty, so the sole purpose of this call is to initialize the circulating supply key
	if err != nil {
		return err
	}
...
// GetCirculatingEquilSupply returns the current circulating supply of equils in the network
func (ui *UTXOIndex) GetCirculatingEquilSupply() (uint64, error) {

	ui.mutex.Lock()
	defer ui.mutex.Unlock()

	return ui.store.getCirculatingEquilSupply()
}
