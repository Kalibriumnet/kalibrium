...
Clients are expected to build RequestMessages and wrap them in KalibriumdMessage. (see messages.proto)

Having received a RequestMessage, (wrapped in a KalibriumdMessage) the RPC server will respond with a
ResponseMessage (likewise wrapped in a KalibriumdMessage) respective to the original RequestMessage.
...
GetCurrentNetworkRequestMessage requests the network Kalibrium is currently running against.
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| payAddress | [string](#string) |  | Which Kalibrium address should the coinbase block reward transaction pay into |
...
| ----- | ---- | ----- | ----------- |
| block | [RpcBlock](#protowire.RpcBlock) |  |  |
| isSynced | [bool](#bool) |  | Whether Kalibrium thinks that it&#39;s synced. Callers are discouraged (but not forbidden) from solving blocks when Kalibrium is not synced. That is because when Kalibrium isn&#39;t in sync with the rest of the network there&#39;s a high chance the block will never be accepted, thus the solving effort would have been wasted. |
| error | [RPCError](#protowire.RPCError) |  |  |
...
GetPeerAddressesRequestMessage requests the list of known Kalibrium addresses in the
...
currently connected to this Kalibrium.
...
| isOutbound | [bool](#bool) |  | Whether this Kalibrium initiated the connection |
| timeOffset | [int64](#int64) |  |  |
| userAgent | [string](#string) |  |  |
| advertisedProtocolVersion | [uint32](#uint32) |  | The protocol version that this peer claims to support |
| timeConnected | [int64](#int64) |  | The timestamp of when this peer connected to this Kalibrium |
...
AddPeerRequestMessage adds a peer to Kalibrium&#39;s outgoing connection list.
This will, in most cases, result in Kalibrium connecting to said peer.
...
parent chain from some startHash to this Kalibrium&#39;s current virtual
...
GetBlockCountRequestMessage requests the current number of blocks in this Kalibrium.
...
ShutDownRequestMessage shuts down this Kalibrium.
...
This call is only available when this Kalibrium was started with `--utxoindex`
...
GetUtxosByAddressesRequestMessage requests all current UTXOs for the given Kalibrium addresses
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| maxEquil | [uint64](#uint64) |  | note: this is a hard coded maxSupply, actual maxSupply is expected to deviate by upto -5%, but cannot be measured exactly. |
| circulatingEquil | [uint64](#uint64) |  |  |
| error | [RPCError](#protowire.RPCError) |  |  |
