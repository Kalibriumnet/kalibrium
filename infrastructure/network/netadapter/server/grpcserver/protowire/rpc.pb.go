...
type GetBlockTemplateRequestMessage struct {
	...
	// Which Kalibrium address should the coinbase block reward transaction pay into
...
type GetBlockTemplateResponseMessage struct {
	...
	// Whether Kalibrium thinks that it's synced.
	// Callers are discouraged (but not forbidden) from solving blocks when Kalibrium is not synced.
	// That is because when Kalibrium isn't in sync with the rest of the network there's a high
...
type GetConnectedPeerInfoMessage struct {
	...
	// Whether this Kalibrium initiated the connection
	...
	// The timestamp of when this peer connected to this Kalibrium
...
type GetCoinSupplyResponseMessage struct {
	...
	MaxEquil         uint64    `protobuf:"varint,1,opt,name=maxEquil,proto3" json:"maxEquil,omitempty"` // note: this is a hard coded maxSupply, actual maxSupply is expected to deviate by upto -5%, but cannot be measured exactly.
	CirculatingEquil uint64    `protobuf:"varint,2,opt,name=circulatingEquil,proto3" json:"circulatingEquil,omitempty"`
	Error            *RPCError `protobuf:"bytes,1000,opt,name=error,proto3" json:"error,omitempty"`
}
...
func (x *GetCoinSupplyResponseMessage) GetMaxEquil() uint64 {
	if x != nil {
		return x.MaxEquil
	}
	return 0
...
func (x *GetCoinSupplyResponseMessage) GetCirculatingEquil() uint64 {
	if x != nil {
		return x.CirculatingEquil
	}
	return 0
}
