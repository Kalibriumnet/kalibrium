...
func commandLoop(argsChan <-chan []string) ([]commandFailure, error) {
	...
	dataDirectoryPath, err := common.TempDir("kalibriumdsanity-Kalibrium-datadir")
	if err != nil {
	...
	for args := range argsChan {
		...
		cmd := exec.Command("Kalibrium", args...)
		cmd.Stdout = common.NewLogWriter(log, logger.LevelTrace, "Kalibrium-STDOUT")
		cmd.Stderr = common.NewLogWriter(log, logger.LevelWarn, "Kalibrium-STDERR")
