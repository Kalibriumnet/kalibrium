...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("Kalibrium-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("Kalibrium datadir: %s", dataDir)

	kalibriumdCmd, err := common.StartCmd("Kalibrium",
		"Kalibrium",
	...
	spawn("startNode-kalibriumdCmd.Wait", func() {
		err := kalibriumdCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("kalibriumdCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes Kalibrium closes ungracefully
				log.Errorf("kalibriumdCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(kalibriumdCmd, "kalibriumdCmd")
...
func mineLoopUntilHavingOnlyOneTipInDAG(rpcClient *rpc.Client, miningAddress util.Address) error {
	...
	kalibriumMinerCmd, err := common.StartCmd("MINER",
		"kalibriumminer",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
		"-s", rpcAddress,
	...
	spawn("Kalibrium-miner-Cmd.Wait", func() {
		err := kalibriumMinerCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
	...
	atomic.StoreUint64(&shutdown, 1)
	killWithSigterm(kalibriumMinerCmd, "kalibriumMinerCmd")
	return nil
}
