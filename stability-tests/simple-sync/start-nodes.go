...
func startNodes() (teardown func(), err error) {
	...
	syncerDataDir, err := common.TempDir("Kalibrium-datadir-syncer")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCER datadir: %s", syncerDataDir)

	syncedDataDir, err := common.TempDir("Kalibrium-datadir-synced")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCED datadir: %s", syncedDataDir)

	syncerCmd, err := common.StartCmd("Kalibrium-SYNCER",
		"Kalibrium",
	...
	syncedCmd, err := common.StartCmd("Kalibrium-SYNCED",
		"Kalibrium",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
