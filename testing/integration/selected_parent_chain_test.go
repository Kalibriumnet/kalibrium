...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of Kalibrium instances
	kalibriumd1, kalibriumd2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := kalibriumd1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In kalibriumd2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, kalibriumd2)
	}

	// Connect the two kalibriumds. This should trigger sync
	// between the two nodes
	connect(t, kalibriumd1, kalibriumd2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := kalibriumd1.rpcClient.GetVirtualSelectedParentChainFromBlock(
