...
func TestUTXOIndex(t *testing.T) {
	// Setup a single Kalibrium instance
	...
	err := Kalibrium.rpcClient.RegisterForUTXOsChangedNotifications([]string{miningAddress1}, func(
	...
	getCoinSupplyResponse, err := Kalibrium.rpcClient.GetCoinSupply()
	if err != nil {
		t.Fatalf("Error Retriving Coin supply: %s", err)
	}

	rewardsMinedEquil := uint64(blockAmountToMine * constants.EquilPerKalibrium * 500)
	getBlockCountResponse, err := Kalibrium.rpcClient.GetBlockCount()
	if err != nil {
		t.Fatalf("Error Retriving BlockCount: %s", err)
	}
	rewardsMinedViaBlockCountEquil := uint64(
		(getBlockCountResponse.BlockCount - 2) * constants.EquilPerKalibrium * 500, // -2 because of genesis and virtual.
	)

	if getCoinSupplyResponse.CirculatingEquil != rewardsMinedEquil {
		t.Fatalf("Error: Circulating supply Mismatch - Circulating Equil: %d Equil Mined: %d", getCoinSupplyResponse.CirculatingEquil, rewardsMinedEquil)
	} else if getCoinSupplyResponse.CirculatingEquil != rewardsMinedViaBlockCountEquil {
		t.Fatalf("Error: Circulating supply Mismatch - Circulating Equil: %d Equil Mined via Block count: %d", getCoinSupplyResponse.CirculatingEquil, rewardsMinedViaBlockCountEquil)
	}
	...
	for i := 0; i < transactionAmountToSpend; i++ {
		...
		_, err = Kalibrium.rpcClient.SubmitTransaction(rpcTransaction, false)
		if err != nil {
	...
	utxosByAddressesResponse, err := Kalibrium.rpcClient.GetUTXOsByAddresses([]string{miningAddress1})
	if err != nil {
...
func buildTransactionForUTXOIndexTest(t *testing.T, entry *appmessage.UTXOsByAddressesEntry) *appmessage.RPCTransaction {
	...
	payeeAddress, err := util.DecodeAddress(miningAddress1, util.Bech32PrefixKalibriumSim)
	if err != nil {
